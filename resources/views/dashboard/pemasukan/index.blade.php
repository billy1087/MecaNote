            ...
            <table class="table table-striped table-bordered">
                ...
                @foreach ($pemasukan as $no => $pmk)
                    <tbody>
                        <tr class="text-center">
                            <td>{{ ++$no }}</td>
                            <td>{{ $pmk->updated_at }}</td>
                            <td>{{ $pmk->nama_pemasukan }}</td>
                            <td>{{ $pmk->jumlah }}</td>
                            <td>{{ $pmk->harga_satuan }}</td>
                            <td>{{ $pmk->keterangan }}</td>
                            <td>{{ $pmk->jumlah * $pmk->harga_satuan }}</td>
                            <td>{{ $pmk->user->nama }}</td>
                            @if (auth()->user()->role == 'admin')
                                <td>
                                    <div class="row my-2 px-3">
                                        <a href="/pemasukan/{{ $pmk->id }}/edit" class="col-6"><i
                                                class="bi bi-pencil-square text-warning" style="font-size: 18px"></i></a>
                                        <form action="/pemasukan/{{ $pmk->id }}" method="post" class="col-6">
                                            @csrf
                                            @method('delete')
